...
test.describe('TC - Flujo completo de generar nuevo codigo QR - Fixer Wallet', () => {

  test('Flujo de login, acceso a cobro por QR', async ({ page }: { page: Page }) => {
    ...
    // Paso 10: Click en "Generar Codigo QR"
    await page.getByRole('button', { name: 'Generar Nuevo Cobro' }).click();
    await page.waitForTimeout(2000);

    // Paso 11: Ingresar Monto a cobrar
    await page.locator('input[placeholder="0.00"]').fill('150');
    await page.waitForTimeout(500);

    // Paso 12: Click en "Generar Codigo QR"
    await page.getByRole('button', { name: 'GENERAR CÓDIGO QR' }).click();
    await page.waitForTimeout(2000);
